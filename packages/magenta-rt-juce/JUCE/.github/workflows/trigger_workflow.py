...
workflow_path = getenv("TRIGGER_WORKFLOW_PATH", ".github/workflows/juce_private_build.yml")
workflow = next(x for x in workflows["workflows"] if x["path"] == workflow_path)
...
github_api_request(
    f"{api_path_prefix}/{workflow['id']}/dispatches", method="POST", data=trigger_data
)
