...
def get_module_description(module_header: Path) -> str:
    with open(module_header) as f:
        content = f.read()
        declaration = content[
            content.find("BEGIN_JUCE_MODULE_DECLARATION") : content.find(
...
def getContentObjectFromPath(root_object: dict, location: str) -> dict:
    ...
    for path_component in path_components[1:-1]:
        if path_component not in contents["dirs"]:
...
def parseNamespaceXml(root_object, filename):
    ...
    for kind, contents_key in (
        ...
        for reference in element.findall(f".//memberdef[@kind='{kind}']"):
            ...
            contents[contents_key][name] = {"url": f"./{namespace_def_id}.html#{anchor}"}
...
def write_module_html_recursive(parent: Element, data: dict):
    for section_key, section_contents in data.items():
        ...
        for section_item_key, section_item_contents in sorted(section_contents.items()):
            ...
            ET.SubElement(
                item, "a", {"href": section_item_contents["url"], "class": "juce-module-class-link"}
...
module_icon = ET.Element(
    "svg", {"fill": "#000000", "viewBox": "0 0 56 56", "class": "juce-module-icon"}
)
ET.SubElement(
    module_icon,
    "path",
...
ET.SubElement(
    toc_div, "p", {"class": "juce-module-toc-desc"}
).text = "Here is a summary of the JUCE modules. To search absolutely everything please use the search bar."
...
for key, contents in sorted(modules.items()):
    ...
    ET.SubElement(
        module_toc_name, "a", {"href": f"#{key}", "class": "juce-module-toc-module-name-link"}
    ).text = key
    ET.SubElement(toc_row, "td", {"class": "juce-module-toc-module-decs"}).text = contents[
        "description"
    ]
...
for key, contents in sorted(modules.items()):
    ...
    ET.SubElement(module_header_div, "span", {"class": "juce-module-desc"}).text = contents[
        "description"
    ]
...
with open("doc/index.html", encoding="utf-8") as f:
    content = f.read()
